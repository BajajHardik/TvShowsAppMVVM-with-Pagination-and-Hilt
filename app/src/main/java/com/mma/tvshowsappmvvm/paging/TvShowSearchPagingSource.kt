...
class TvShowSearchPagingSource(private val moviesApi: MoviesApi,
                               private val query:String): androidx.paging.PagingSource<Int, TvShow>() {
    override suspend fun load(params: LoadParams<Int>): LoadResult<Int, TvShow> {
        return try{
            val position = params.key ?: 1
            val response = moviesApi.searchPopularTvShows(query,position)
